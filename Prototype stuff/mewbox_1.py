...
class ToolboxSwapper(tk.Frame):
    ...
    def _csv_to_xml(self):
        ...
        # Detect which columns are text (need CDATA) vs numeric (plain tag)
        text_cols = self._detect_text_columns(headers, csv_rows)
        self._log(f"Text/CDATA columns: {', '.join(text_cols) if text_cols else 'none'}")

        def _fmt_field(tag, val):
            """Format a single field value. Text columns always use CDATA;
            null/blank text fields use <![CDATA[ ]]> (a single space, per TO convention)."""
            if tag in text_cols:
                inner = val if val else " "
                return f"<{tag}><![CDATA[{inner}]]></{tag}>"
            else:
                return f"<{tag}>{val}</{tag}>"

        # Build ROW blocks — no indentation, matching toolbox raw output style
        row_blocks = []
        for r in csv_rows:
            parts = ["<ROW>"]
            for h in headers:
        ...
        if self._base_path:
            ...
            fi_lines = "\n".join(
                f'<FIELDINFO Name="{h}" IsKey="{"1" if i==0 else "0"}" DataType="{"STRING" if h in text_cols else "INT"}"></FIELDINFO>'
                for i, h in enumerate(headers)
            )
            out_fname = tname + ".xml"
            out_text = (
