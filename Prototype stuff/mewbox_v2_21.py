...
class Tool1(tk.Frame):
    ...
    def _build_config_screen(self):
        ...
        myshop_glc_var = tk.StringVar(value=str(_s_myshop.get("myshop_glc_start", _last_glc_t1 + 1)))
        _row("goods_list_code start:", myshop_glc_var, tip="Starting goods_list_code/parents_list_code for libcmgds_e. Increments per box. Separate from version_code.")
        ...
        if _myshop_xml_blocks:
            ...
            for cfg in self.box_configs:
                if cfg.get("myshop_enabled"):
                    ...
                    try: _set_last_id("t1_goods_list_code", int(cfg.get("myshop_glc_start", 21040)) + 1)
                    except: pass
...
def _build_box_myshop_outputs(box_configs):
    ...
    # goods_list_code counter — each enabled box consumes one value.
    # Seed from the first enabled box's glc_start; subsequent boxes increment.
    _glc_counter = None
    ...
    for cfg in box_configs:
        ...
        # goods_list_code: seed from this box's glc_start on first box,
        # then keep incrementing across all boxes in this batch.
        if _glc_counter is None:
            try: _glc_counter = int(cfg.get("myshop_glc_start", 21000))
            except: _glc_counter = 21000
        glc = _glc_counter
        _glc_counter += 1
