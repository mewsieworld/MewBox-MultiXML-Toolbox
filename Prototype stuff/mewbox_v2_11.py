    ...
    def _build_editor(self, prefill=None):
        ...
        tk.Label(s_opt,
                 text="  Click each button to open the corresponding generator pre-filled.\n"
                      "  Uncheck any step you don't need. Work through them in order.",
                 bg=BG, fg=FG_GREY, font=("Consolas", 8)).pack(anchor="w", padx=10, pady=4)

        # Step enable checkboxes
        steps_row = tk.Frame(s_opt, bg=BG); steps_row.pack(fill="x", padx=10, pady=2)
        self._do_itemparam = tk.BooleanVar(value=pf.get("do_itemparam", True))
        self._do_set       = tk.BooleanVar(value=pf.get("do_set",       True))
        self._do_box       = tk.BooleanVar(value=pf.get("do_box",       True))
        self._do_shop_ce   = tk.BooleanVar(value=pf.get("do_shop_ce",   False))
        for text, var in [
            ("① ItemParam", self._do_itemparam),
            ("② Set",       self._do_set),
            ("③ Box",       self._do_box),
            ("④ Shop/CE",   self._do_shop_ce),
        ]:
            tk.Checkbutton(steps_row, text=text, variable=var,
                           bg=BG, fg=FG, selectcolor=BG3, activebackground=BG,
                           font=("Consolas", 9)).pack(side="left", padx=8)
            ...
            try:
                for i, (_, _, _, cls_) in enumerate(TOOLS):
                    if cls_ is Tool1:
                        self.root._switch_tool(i)
                        tool = self.root._tool_instances.get(i)
                        if tool and hasattr(tool, "_build_config_screen"):
                            _save_settings("t1_box", pre)
...
class Tool8(tk.Frame):
    ...
    def _build_manual_editor(self, prefill=None):
        ...
        v_set_id   = tk.StringVar(value=str(pf.get("set_id",  int(last_set_id) + 1)))
