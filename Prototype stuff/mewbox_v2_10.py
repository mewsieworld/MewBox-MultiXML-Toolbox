...
class Tool18(tk.Frame):
    ...
    def _build_editor(self, prefill=None):
        ...
        # ── ⑤ ItemParam Config  (hidden — passed to ItemParam Generator) ───────
        # These vars are stored here so they pre-fill the ItemParam Generator.
        # They are NOT shown as a big inline form. The user configures them
        # properly inside the ItemParam Generator when it opens.
        ...
        # ItemParam-specific config stored as hidden vars — all pre-filled into Tool6
        ...
        self._fash_recycle_var      = tk.IntVar(value=pf.get("fash_recycle", 0))
        ...
        # Shop/Exchange/Compound config stored as dicts — passed to those generators
        ...
        # ── ⑤ Launch Generators ──────────────────────────────────────────
        s_opt = sec("  ⑤ Launch Generators  (pre-fills each with your session data)  ")
        tk.Label(s_opt,
                 text="  Click each button to open the corresponding generator pre-filled.\n"
                      "  Work through them in order: ItemParam → Set → Box → Shop → Exchange/Compound.",
                 bg=BG, fg=FG_GREY, font=("Consolas", 8)).pack(anchor="w", padx=10, pady=4)

        launch_row = tk.Frame(s_opt, bg=BG); launch_row.pack(anchor="w", padx=10, pady=6)

        def _launch_itemparam():
            ...
            for rf, vid, vn, vcmt in self._item_row_widgets:
                if rf.winfo_exists() and vn.get().strip():
                    pfn = getattr(vid, "_part_fn", self._partfile_shared_var.get().strip())
                    pieces.append({"name": vn.get().strip(),
                                   "comment": vcmt.get().strip(),
                                   "item_id": vid.get().strip(),
                                   "partfn": pfn})
            if not pieces:
                messagebox.showwarning("No Pieces",
                    "Add fashion pieces in ⑥ Fashion Pieces before launching.", parent=self.root)
                return
            char = self._char_var.get()
            _, _, _, _ = _CHAR_DATA.get(char, (1,2,8,128))
            ...
            opt_str = "/".join(["2","16"] + opt_flags) if not opt_flags else "/".join(["2","16"] + opt_flags)
            # NCash from recycling ticket calc
            ...
            # First piece cfg for Tool6 pre-fill
            p0 = pieces[0]
            cls2, typ2, sub2 = _fashion_detect_slot(p0["name"])
            pfn0 = _sanitise_xml_path(p0.get("partfn",""))
            galder = getattr(self,"_galder_val",tk.StringVar(value="0")).get() if getattr(self,"_use_galder",tk.BooleanVar()).get() else "0"
            start_id = _get_last_id("t18_fashion_item", 0)
            cfg = {
                "id":              str(start_id + 1) if start_id else "",
                "class_val":       str(cls2),
                "type_val":        str(typ2),
                ...
                "value":           galder,
                "ncash":           ncash_val,
                # Stash extra context so Tool6 can show a "fashion batch" mode
            ...
            # Switch to Tool6 (ItemParam Generator)
            try:
                for i, (_, _, _, cls_) in enumerate(TOOLS):
                    if cls_ is Tool6:
                        self.root._switch_tool(i); return
            except Exception as e:
        ...
        def _launch_set():
            ...
            for rf, vid, vn, vcmt in self._item_row_widgets:
                if rf.winfo_exists() and vid.get().strip():
                    items.append(vid.get().strip())
            set_id = self._set_id.get().strip()
            set_name = self._set_name.get().strip()
            if not set_id:
                messagebox.showwarning("No Set ID", "Enter a Set ID in ④ Set & Box first.", parent=self.root); return
            pre = {"last_id": set_id, "set_name": set_name}
            for i, iid in enumerate(items[:8]):
                pre[f"item{i}"] = iid
        ...
        def _launch_box():
            ...
            if not box_id:
                messagebox.showwarning("No Box ID", "Enter a Box ID in ④ Set & Box first.", parent=self.root); return
            items = []
            for rf, vid, vn, vcmt in self._item_row_widgets:
                if rf.winfo_exists() and vid.get().strip():
                    items.append({"id": vid.get().strip(), "name": vn.get().strip()})
            fn = _sanitise_xml_path(self._filename_var.get().strip())
            pre = {
                "id":       box_id,
                ...
                "file_name": fn,
                "bundle_num": self._bundlenum_var.get().strip() or "0",
                "items":    items,
            }
        ...
        mk_btn(launch_row, "① ItemParam Generator", _launch_itemparam,
               color=ACC6, fg=BG2, font=("Consolas", 10, "bold")).pack(side="left", padx=4)
        mk_btn(launch_row, "② Set Generator", _launch_set,
               color=ACC8, fg=BG2, font=("Consolas", 10, "bold")).pack(side="left", padx=4)
        mk_btn(launch_row, "③ Box Generator", _launch_box,
               color=ACC1, fg=BG2, font=("Consolas", 10, "bold")).pack(side="left", padx=4)
        mk_btn(launch_row, "④ Shop / Exchange / Compound", _launch_shop_ce,
               color=ACC7, fg=BG2, font=("Consolas", 10, "bold")).pack(side="left", padx=4)

        tk.Label(s_opt,
                 text="  ① Open ItemParam Generator for each piece\n"
                      "  ② Open Set Generator with piece IDs pre-filled\n"
                      "  ③ Open Box Generator with box ID and piece list pre-filled\n"
                      "  ④ Open Shop/Exchange/Compound with item IDs ready to use",
        ...
        mk_btn(bot, "💾  Save Session", self._save_session,
               color=GREEN, fg=BG2,
               font=("Consolas", 11, "bold")).pack(side="right", padx=16, pady=8)
