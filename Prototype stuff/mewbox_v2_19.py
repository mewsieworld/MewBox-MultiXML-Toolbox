...
class Tool1(tk.Frame):
    ...
    def _build_output_screen(self, _compound_rows=None, _exchange_rows=None):
        ...
        def _add_ce():
            items = [{"id": c["id"], "name": c.get("name",""), "comment": c.get("comment","")}
                     for c in self.box_configs]
            def _on_done(comp_cfgs, exch_cfgs, shop_cfgs):
                for cfg in comp_cfgs:
                    _compound_rows.append((build_compound_row(cfg),
                                           build_compound_location_row(cfg["compound_id"])))
                    try: _set_last_id("compound", int(cfg["compound_id"]))
                    except: pass
                for cfg in exch_cfgs:
                    _exchange_rows.append((build_exchange_row(cfg),
                                           build_exchange_location_row(cfg["exchange_id"])))
                    try: _set_last_id("exchange", int(cfg["exchange_id"]))
                    except: pass
                if not hasattr(self, "_extra_shop_rows"):
                    self._extra_shop_rows = []
                for cfg in shop_cfgs:
                    self._extra_shop_rows.append(
                        build_shop_row(cfg["id"], cfg.get("count","100"), cfg.get("price","0")))
                self.session.compound_rows = list(_compound_rows)
                self.session.exchange_rows = list(_exchange_rows)
                if comp_cfgs or exch_cfgs or shop_cfgs:
...
class Tool2(tk.Frame):
    ...
    def _build_load_screen(self):
        ...
        # ── Step 3: Nested mode ───────────────────────────────────────────
        nest_sec=mk_section(self,"  Step 3 — Lookup Mode  ")
        tk.Label(nest_sec,
        ...
        # ── Step 4: Mode ──────────────────────────────────────────────────
        mode_frm=mk_section(self,"  Step 4 — Adjustment Mode  ")
    ...
    def _run_automatic(self, matched, rate, count):
        ...
        def replace_row(m):
            ...
            cfg={"type":2,"drop_cnt":len(slots),"slots":slot_cfgs}
