...
class Tool18(tk.Frame):
    ...
    def _build_editor(self, prefill=None):
        ...
        # ── ② Pricing ────────────────────────────────────────────────────
        s_price = sec("  ② Pricing  ")
        pr = tk.Frame(s_price, bg=BG); pr.pack(fill="x", padx=10, pady=6)
        self._price_mode = tk.StringVar(value=pf.get("price_mode", "none"))
        for lbl, val in [("None", "none"), ("MyShop (NCash)", "myshop"),
        ...
        self._price_val   = tk.StringVar(value=pf.get("price_val", "0"))
        self._cash_price  = tk.StringVar(value=pf.get("cash_price", "0"))
        pr2 = tk.Frame(s_price, bg=BG); pr2.pack(fill="x", padx=10, pady=3)
        tk.Label(pr2, text="Price value:", bg=BG, fg=FG, font=("Consolas",9), width=22, anchor="w").pack(side="left")
        tk.Entry(pr2, textvariable=self._price_val, width=14, bg=BG3, fg=FG,
                 insertbackground=FG, font=("Consolas",9), relief="flat").pack(side="left", padx=4)
        tk.Label(pr2, text="(goods_discount_price for MyShop · Ncash for Galder)",
                 bg=BG, fg=FG_GREY, font=("Consolas", 8)).pack(side="left")
        pr3 = tk.Frame(s_price, bg=BG); pr3.pack(fill="x", padx=10, pady=3)
        tk.Label(pr3, text="goods_cash_price:", bg=BG, fg=FG, font=("Consolas",9), width=22, anchor="w").pack(side="left")
    ...
    def _collect_state(self):
        ...
        return {
            ...
            "price_val":     self._price_val.get(),
            "cash_price":    self._cash_price.get(),
    ...
    def _generate(self):
        ...
        for it in items_raw:
            n = it["name"]
            if pos == "before" and prefix:
                n = prefix + n
            elif pos == "after" and suffix:
                n = n + suffix
            elif pos == "both":
                n = prefix + n + suffix
            items.append({**it, "name": n})
        ...
        # Paths are stored as-is; CDATA wrapper keeps backslashes safe in XML output
        ...
        if set_id:
            slots = "".join(
                f"<Item{i}>{it['item_id']}</Item{i}> <!-- {it['name']} -->\n"
                for i, it in enumerate(items[:MAX_FASHION_PIECES]))
            cmset_row = (f"<ROW>\n<ID>{set_id}</ID>\n"
        ...
        if self._gen_exchange.get() and self._exch_fields:
        ...
        if self._gen_compound.get() and self._comp_fields:
        ...
        if price_mode == "myshop":
            ...
            libcmgds = (
                ...
                f"Goods_category=1\nGoods_category0=15\ngoods_char_type=15\n"
                f"goods_cash_price={cash_price}\ngoods_discount_price={price_val}\n"
        ...
        if price_mode == "myshop":
            ...
            sql_block = "\n".join(
                ...
                f"VALUES ({iid}, {price_val}, {price_val}, {sex}, {char_sql_type}, {cat1}, 15, 1);"
                for iid in all_ids
            )

        # Auto-save session on generate
    ...
    def _show_output(self, itemparam_rows, cmset_row, box_row, shop_rows,
        ...
        if compound_row:
            ...
            make_output_tab(nb, "Compound_Location",compound_loc,"fashion_compound_loc.xml",self.root)
        if price_mode == "myshop":
            if libcmgds:
    ...
    def _export_fashion(self, ip_text, box_row, cmset_row, shop_rows,
        ...
        _w(lib_dir, "fashion_compound_loc.xml", compound_loc)
        if price_mode == "myshop":
