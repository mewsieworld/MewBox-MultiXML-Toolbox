...
def adjust_row_to_100pct(block):
    """Set Type=2, DropCnt=real item count, all DropRate_# for real items = 100, zeros stay 0."""
    ...
    def fix_rate(m):
        full   = m.group(0)
        idx    = int(re.search(r'DropId_(\d+)', full).group(1))
        id_val = re.search(r'<DropId_\d+>(\d+)</DropId_\d+>', full).group(1)
        rate   = "100" if id_val != "0" else "0"
        full   = re.sub(r'<DropRate_\d+>.*?</DropRate_\d+>',
                        f'<DropRate_{idx}>{rate}</DropRate_{idx}>', full)
        return full

    # Replace each triplet together
    combined = re.sub(
        r'(<DropId_(\d+)>.*?</DropId_\2>)\s*(<DropRate_\2>.*?</DropRate_\2>)\s*(<ItemCnt_\2>.*?</ItemCnt_\2>)',
        lambda m: (
            m.group(1) + "\n" +
            re.sub(r'<DropRate_(\d+)>.*?</DropRate_\1>',
                   lambda r: f'<DropRate_{r.group(1)}>{"100" if get_tag_val(m.group(1).replace("DropId_","DropId_").split(">")[1].split("<")[0], "") != "0" else "0"}</DropRate_{r.group(1)}>',
                   m.group(3)) + "\n" +
            m.group(4)
        ),
    ...
    for i in range(20):
        ...
        rate   = "100" if id_val and id_val != "0" else "0"
        block  = set_tag_val(block, f"DropRate_{i}", rate)
    return block
