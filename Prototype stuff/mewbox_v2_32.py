...
class Tool2(tk.Frame):
    ...
    def _on_continue(self):
        ...
        if nested:
            ...
            # Recursively find true leaf IDs (not boxes)
            leaf_ids, traversed = extract_drop_ids_recursive(self.xml_text, csv_ids)

            if not leaf_ids:
                messagebox.showwarning("No Leaf Items Found",
                    f"Recursive search found no leaf items.\n"
                    f"Started from {len(csv_ids)} CSV IDs, traversed {len(traversed)} intermediate boxes.\n\n"
                    "If your CSV IDs ARE the rows to adjust directly, use Direct mode instead."); return

            # Match leaf IDs against XML rows
            matched=[]; seen=set()
            for row in ROW_RE.findall(self.xml_text):
                rid=_get_tag(row,"Id")
                if rid in leaf_ids and rid not in seen:
                    # Name/meta: use from csv_meta if available, else blank
                    nm2, meta2, grp = csv_meta.get(rid, (rid, {}, ""))
                    matched.append((rid, nm2, row, meta2, grp)); seen.add(rid)
            if not matched:
                messagebox.showwarning("No Matches",
                    f"Recursive search found {len(leaf_ids)} leaf item IDs but none matched "
                    f"any <Id> row in PresentItemParam2.xml.\n\n"
                    f"Leaf sample: {sorted(leaf_ids)[:5]}\n\n"
                    "These leaf items may be in ItemParam.xml, not PresentItemParam2.xml. "
                    "They do not have PresentItemParam2 rows to adjust."); return
        else:
