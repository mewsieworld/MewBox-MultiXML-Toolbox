...
class Tool6(tk.Frame):
    ...
    def _build_editor(self):
        ...
        # ═════════════════════════════════════════════════════════════════
        s_pivot = sec("  PivotID & PaletteId  (Default: 0) ")
        ...
        # ═════════════════════════════════════════════════════════════════
        s4 = sec("  Options  (eItemOption — select flags, output is slash-separated e.g. 1/2/256)")

        # Restore saved options from slash-separated raw string
        saved_opts_raw = s.get("options_raw_manual", "0")
        _saved_active = set()
        for _p in str(saved_opts_raw).replace(",", "/").split("/"):
            try: _saved_active.add(int(_p.strip()))
            except: pass
        # Fall back to boolean list if raw is all zeros but list has data
        ...
        opts_vars = [tk.BooleanVar(value=(fval in _saved_active))
                     for fval, _ in _OPTIONS_FULL]
        ...
        opt_preview = tk.Label(dd_opts_frm, text="Value: ", bg=BG, fg=GREEN,
                               font=("Consolas", 9))
        ...
        tk.Label(r_mo, text="Options (slash-sep):", width=22, anchor="w",
                 bg=BG, fg=FG, font=("Consolas", 9)).pack(side="left")
        ...
        # Sync manual -> checkboxes (slash-separated or single int)
        ...
        gnd_ent = lbl_entry(s7, "GroundFlags:  (Default: 0)", v_ground, 10, _TOOLTIPS["GroundFlags"])
        sys_ent = lbl_entry(s7, "SystemFlags:  (Default: 0)", v_system, 10, _TOOLTIPS["SystemFlags"])
        ...
        lbl_entry(s9, "Depth:        (Default: 0)", v_depth,     10, _TOOLTIPS["Depth"])
        ...
        lbl_entry(s11, "SetItemID:   (Default: 0)",v_setitem,   8, _TOOLTIPS["SetItemID"])
        lbl_entry(s11, "ReformCount: (Default: 0)",v_reform,    8, _TOOLTIPS["ReformCount"])
