...
class NCashUpdaterApp(tk.Tk):
    ...
    def _build_load_screen(self):
        ...
        def load_xml():
            ...
            choice_win.title("Load ItemParam")
            choice_win.geometry("420x160")
            choice_win.configure(bg="#1e1e2e")
            choice_win.grab_set()
            tk.Label(choice_win, text="Load a single file or scan an entire folder?",
                     bg="#1e1e2e", fg="#cdd6f4", font=("Consolas", 10)).pack(pady=16)
            bf = tk.Frame(choice_win, bg="#1e1e2e"); bf.pack(pady=4)

            def pick_file():
                choice_win.destroy()
                path = filedialog.askopenfilename(
                    title="Select ItemParam XML",
                ...
                for fname in os.listdir(folder):
                    if fname.lower().endswith(".xml"):
                        try:
                            ...
                            pass
                if not combined:
                    messagebox.showerror("Error", "No XML files found in that folder.")
                    return
                self.xml_text = "\n".join(combined)
                xml_status.set(f"✓  {len(found)} file(s) from {os.path.basename(folder)}"
                               f"  ({', '.join(found[:3])}{'…' if len(found)>3 else ''})")

            tk.Button(bf, text="📄  Single File", command=pick_file,
                      bg="#313244", fg="#cdd6f4", font=("Consolas", 10),
                      relief="flat", padx=14, pady=6).pack(side="left", padx=8)
            tk.Button(bf, text="📁  Folder (all XMLs)", command=pick_folder,
