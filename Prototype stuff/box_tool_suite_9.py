...
_SETTINGS_PATH = os.path.join(os.path.expanduser("~"), ".box_tool_suite_t6.json")

def _load_t6_settings():
    try:
...
class Tool6(tk.Frame):
    ...
    def _build_editor(self):
        ...
        v_id    = tk.StringVar(value=s.get("id", ""))
        ...
        def _generate():
            ...
            # Save settings — first run is now complete
        ...
        def _generate_and_next():
            ...
            self._show_output(xml, cfg, auto_next=True)
    ...
    def _show_output(self, xml, cfg, auto_next=False):
        ...
        # If called from Generate & Next, immediately proceed to editor for next ID
        if auto_next:
