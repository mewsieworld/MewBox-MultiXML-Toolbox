...
def build_presentparam_row(box_id, items, ptype, drop_cnt, default_rate):
    ...
    for i in range(20):
        if i < len(items):
            ...
            did, drate = 0, 0
        item_cnt = 1 if i < len(items) else 0
        lines += [f"<DropId_{i}>{did}</DropId_{i}>",
                  f"<DropRate_{i}>{drate}</DropRate_{i}>",
                  f"<ItemCnt_{i}>{item_cnt}</ItemCnt_{i}>"]
    lines.append("</ROW>")
    return "\n".join(lines)
...
class BoxGeneratorApp(tk.Tk):
    ...
    def _build_config_screen(self):
        ...
        for it in grp["items"]:
            if present_type_var.get() == 2:
                initial = "100"
            elif it["extra"] is not None:
                initial = str(it["extra"])
            else:
                try:
        ...
        # ── DropCnt Flexible / Manual toggle ──────────────────────────────
        dc_mode_var   = tk.StringVar(value=s.get("dc_mode",   "flexible"))
        rate_mode_var = tk.StringVar(value=s.get("rate_mode", "manual"))
        dc_mode_frm = tk.Frame(sec_pres, bg="#1e1e2e")
        dc_mode_frm.pack(anchor="w", padx=8, pady=(2,0))
        tk.Label(dc_mode_frm, text="DropCnt:", bg="#1e1e2e", fg="#cdd6f4",
                 font=("Consolas",9)).pack(side="left")
        tk.Radiobutton(dc_mode_frm, text="Flexible  (= item count)",
                       variable=dc_mode_var, value="flexible",
                       bg="#1e1e2e", fg="#cdd6f4", selectcolor="#313244",
                       activebackground="#1e1e2e", font=("Consolas",9),
                       command=lambda: _toggle_dc_mode()).pack(side="left", padx=6)
        tk.Radiobutton(dc_mode_frm, text="Manual",
                       variable=dc_mode_var, value="manual",
                       bg="#1e1e2e", fg="#cdd6f4", selectcolor="#313244",
                       activebackground="#1e1e2e", font=("Consolas",9),
        ...
        dc_lbl = tk.Label(dc_frm, text="DropCnt value:", bg="#1e1e2e", fg="#cdd6f4", font=("Consolas",9))
        ...
        # ── DropRate mode: Manual vs Distributive ─────────────────────────
        dr_mode_frm = tk.Frame(sec_pres, bg="#1e1e2e")
        dr_mode_frm.pack(anchor="w", padx=8, pady=(4,0))
        tk.Label(dr_mode_frm, text="DropRate:", bg="#1e1e2e", fg="#cdd6f4",
                 font=("Consolas",9)).pack(side="left")
        tk.Radiobutton(dr_mode_frm, text="Manual",
                       variable=rate_mode_var, value="manual",
                       bg="#1e1e2e", fg="#cdd6f4", selectcolor="#313244",
                       activebackground="#1e1e2e", font=("Consolas",9),
        ...
        dr_dist_frm = tk.Frame(sec_pres, bg="#1e1e2e")
        # not packed here — _toggle_rate_mode() controls visibility
        tk.Label(dr_dist_frm, text="Default DropRate:", bg="#1e1e2e", fg="#cdd6f4",
                 font=("Consolas",9)).pack(side="left")
        dr_dist_ent = tk.Entry(dr_dist_frm, textvariable=default_rate_var, width=6,
                               bg="#313244", fg="#cdd6f4", insertbackground="#cdd6f4",
                               font=("Consolas",9), relief="flat")
        dr_dist_ent.pack(side="left", padx=6)
        dr_dist_preview = tk.Label(dr_dist_frm, text="", bg="#1e1e2e", fg="#6c7086",
                                   font=("Consolas",8))
        dr_dist_preview.pack(side="left")

        def _apply_dist_rate(*_):
            """When distributive mode, push default_rate_var into all item entries."""
            if rate_mode_var.get() == "distributive":
                try:   val = str(int(default_rate_var.get()))
                except: val = "50"
                for var in item_rate_vars:
                    var.set(val)
                dr_dist_preview.config(text=f" → all entries set to {val}")
            else:
        ...
        def _toggle_rate_mode():
            ...
            if is_dist:
                dr_dist_frm.pack(anchor="w", padx=8, pady=2)
                _apply_dist_rate()
        ...
        def _toggle_dc_mode():
            is_manual = (dc_mode_var.get() == "manual")
            if is_manual:
                ...
                dc_lbl.pack_forget()
                dc_ent.pack_forget()
            # If distributive + manual, cap DropCnt warning at 6
            if present_type_var.get() == 2 and is_manual:
                rate_note.config(
                    text="  Distributive Manual: DropCnt max 6 recommended.")
            elif present_type_var.get() == 2:
        ...
        def toggle_drop_fields(*_):
            # NOTE: drop type (Random/Distributive) is separate from DropRate mode.
            # This only updates the rate_note hint; entry state is managed by _toggle_rate_mode.
            _toggle_dc_mode()
            rate_note.config(text="")  # rate_mode handles its own note via dr_dist_preview

        present_type_var.trace_add("write", toggle_drop_fields)
        _toggle_dc_mode()      # apply dc_mode first
        toggle_drop_fields()   # then apply present type (calls _toggle_dc_mode again, fine)
        _toggle_rate_mode()    # apply rate_mode (manual/distributive) on top
        ...
        def gather_config():
            ...
            return {
                ...
                "drop_cnt":         drop_cnt_var.get() or "1" if dc_mode_var.get() == "manual" else str(len(grp["items"])),
                "default_rate":     default_rate_var.get() or "50",
                "remember_present": remember_present.get(),
                "dc_mode":          dc_mode_var.get(),
                "rate_mode":        rate_mode_var.get(),
            }
    ...
    def _automate_remaining(self, last_cfg):
        ...
        for i in range(self.current_group_idx, len(self.groups)):
            ...
            cfg["item_rates"] = [
                100 if is_distrib else (it["extra"] if it["extra"] is not None else default_rate)
                for it in grp["items"]
            ]
    ...
    def _build_output_screen(self):
        ...
        for cfg in self.box_configs:
            ...
            # RecycleExceptItem: add row when None (0) or Non-Recyclable (8388608)
            if cfg.get("opt_recycle", 0) in (0, 8388608):
                recycle_except_rows.append(
                    build_recycle_except_row(cfg["id"], cfg["name"]))
        ...
        if recycle_except_rows:
            make_tab("RecycleExceptItem.xml rows", recycle_except_rows, "RecycleExceptItem_rows.xml")
