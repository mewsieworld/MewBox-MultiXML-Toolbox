...
class NCashUpdaterParentApp(tk.Tk):
    ...
    def _build_sub_configure_screen(self):
        ...
        if self.sub_mode_var.get() == "uniform":
            # If every sub_item already has a value from the CSV box_ticket_cost
            # column, there's nothing to ask — apply and proceed directly.
            all_prefilled = all(
                it.get("ticket_cost") is not None or it.get("ncash_direct") is not None
                for it in sub_items
            )
            if all_prefilled:
