...
class Tool7(tk.Frame):
    """Compound / Exchange Generator — standalone tool."""
    ...
    def __init__(self, parent, root, session):
        ...
        self._groups        = []   # list of {group_name, rows:[cfg], mode}
    ...
    def _build_start_screen(self):
        ...
        tk.Label(info,
            text=(
                "Load a CSV or Excel spreadsheet with compound/exchange entries.\n"
                "Both types can exist in the same file (use group column headers).\n\n"
                "  Compound columns: CompoundID, Name, Comment, ResLv, ResID1-3,\n"
                "    ReqID1-5, ReqNum1-5, Probability, Fee, WasteItem\n"
                "  Exchange columns: ExchangeID, Name, Comment, ResLv, ResID1-3,\n"
                "    ReqID1-5, ReqNum1-5, Fee\n\n"
                "  Group headers: any non-field column becomes a batch group name.\n"
                "  Excel: multiple sheets are each treated as separate groups."
            ),
            bg=BG2, fg=FG, font=("Consolas", 8), justify="left",
        ...
        if self.session.compound_rows or self.session.exchange_rows:
            def _import_session_ce():
                ...
                messagebox.showinfo("Session Imported",
                    f"Imported from session:\n  {n_c} compound row(s)\n  {n_e} exchange row(s)")
                self._build_output_screen()
            mk_btn(bf, "⬇  Import from Session",
                   _import_session_ce, color=BG4).pack(side="left", padx=8)

        # Template downloads
        tpl_frm = tk.Frame(center, bg=BG); tpl_frm.pack(pady=4)
        def _save_tpl(mode):
            cols = list((_COMPOUND_DEFAULTS if mode=="compound" else _EXCHANGE_DEFAULTS).keys())
            path = filedialog.asksaveasfilename(
        ...
        mk_btn(tpl_frm, "📄 Compound template", lambda: _save_tpl("compound"),
               color=BG4, font=("Consolas",8)).pack(side="left", padx=6)
        mk_btn(tpl_frm, "📄 Exchange template", lambda: _save_tpl("exchange"),
               color=BG4, font=("Consolas",8)).pack(side="left", padx=6)

        # Existing rows status
        if self._compound_rows or self._exchange_rows:
            n_c, n_e = len(self._compound_rows), len(self._exchange_rows)
            status_lbl = f"Session: {n_c} compound + {n_e} exchange rows pending"
            tk.Label(center, text=status_lbl, bg=BG, fg=GREEN,
    ...
    # ── Manual entry (choose compound or exchange first) ──────────────────
    def _build_manual_entry(self):
        """Ask compound or exchange, then show the appropriate form."""
        win = tk.Toplevel(self.root)
        win.title("New Entry"); win.configure(bg=BG)
        win.resizable(False, False); win.grab_set()

        tk.Label(win, text="What type of entry?", bg=BG, fg=FG,
                 font=("Consolas", 13, "bold"), pady=14).pack(padx=28)
        ...
        def _compound():
            ...
            def _done(cfg):
                ...
                self._build_output_screen()
            _show_compound_form(self.root, "", "", "", _done)
        ...
        def _exchange():
            ...
            def _done(cfg):
                ...
                self._build_output_screen()
            _show_exchange_form(self.root, "", "", "", _done)

        mk_btn(bf, "⚗  Compound", _compound,
               color=ACC1, fg=BG2, font=("Consolas", 11, "bold"),
               padx=18, pady=10).grid(row=0, column=0, padx=10, pady=6)
        mk_btn(bf, "🔄  Exchange", _exchange,
               color=ACC2, fg=BG2, font=("Consolas", 11, "bold"),
               padx=18, pady=10).grid(row=0, column=1, padx=10, pady=6)
        mk_btn(win, "Cancel", win.destroy, color=BG4).pack(pady=(0, 12))
        win.wait_window()
    ...
    def _import_from_file(self):
        path = filedialog.askopenfilename(
            title="Import Compound / Exchange spreadsheet",
            filetypes=[
        ...
        def _do_import(mode):
            ...
            try:
                if ext in (".xlsx", ".xlsm", ".xls"):
                    ...
                    for sh_name in wb.sheetnames:
                        ...
                        groups = _parse_ce_grouped_csv(csv_text, mode)
                        all_groups.extend(groups)
                else:
            ...
            if not all_groups:
                messagebox.showwarning("No data", "No valid rows found in the file."); return

            # Show preview and confirm
            self._preview_groups(all_groups, path)
        ...
        mk_btn(bf, "⚗  Compound", lambda: _do_import("compound"),
               ...
               padx=14, pady=8).grid(row=0, column=0, padx=8, pady=4)
        mk_btn(bf, "🔄  Exchange", lambda: _do_import("exchange"),
               color=ACC2, fg=BG2, font=("Consolas",10,"bold"),
               padx=14, pady=8).grid(row=0, column=1, padx=8, pady=4)
        mk_btn(bf, "Both (detect by column)", _do_both,
               color=BG3, font=("Consolas",9),
               padx=10, pady=8).grid(row=1, column=0, columnspan=2, padx=8, pady=4)
        mk_btn(win, "Cancel", win.destroy, color=BG4).pack(pady=(0, 12))
        win.wait_window()

    def _import_mixed_file(self, path, ext):
        """Import a file that may contain both compound and exchange groups (auto-detect)."""
        try:
            if ext in (".xlsx", ".xlsm", ".xls"):
                if not _HAVE_OPENPYXL:
                    messagebox.showerror("Missing library",
                    ...
                    return
                wb = openpyxl.load_workbook(path, data_only=True)
                # Try compound then exchange on each sheet
                all_groups = []
                for sh_name in wb.sheetnames:
                    csv_text = _sheet_to_csv(wb[sh_name])
                    # Try compound first
                    ...
                    if not grps:
                        grps = _parse_ce_grouped_csv(csv_text, "exchange")
                ...
                if not all_groups:
                    all_groups = _parse_ce_grouped_csv(csv_text, "exchange")
        except Exception as e:
            messagebox.showerror("Import Error", str(e)); return
        if not all_groups:
    ...
    def _preview_groups(self, groups, filepath):
        """Show a preview window for all parsed groups before committing."""
        win = tk.Toplevel(self.root)
        win.title(f"Import Preview — {len(groups)} group(s)")
        win.configure(bg=BG); win.geometry("900x600"); win.grab_set()

        hdr = tk.Frame(win, bg=BG2); hdr.pack(fill="x")
        tk.Label(hdr, text=f"📥  Import Preview — {os.path.basename(filepath)}",
                 bg=BG2, fg=self.ACC, font=("Consolas",12,"bold"), pady=8).pack(side="left", padx=14)
        tk.Label(hdr, text=f"{sum(len(g['rows']) for g in groups)} total rows in {len(groups)} group(s)",
        ...
        for g in groups:
            mode = g["mode"]
            color = ACC1 if mode == "compound" else ACC2
            gf = tk.LabelFrame(C, text=f"  {g['group_name']}  [{mode}]  ({len(g['rows'])} rows)",
                                bg=BG, fg=color, font=("Consolas",9,"bold"), bd=1, relief="groove")
            gf.pack(fill="x", padx=8, pady=4)
            id_key = "compound_id" if mode == "compound" else "exchange_id"
            cols = [id_key, "name", "res_lv", "res_id1", "req_id1", "req_num1"]
            if mode == "compound": cols.append("probability")
        ...
        nav = tk.Frame(win, bg=BG2); nav.pack(fill="x", side="bottom")
        tk.Label(nav, text="Review above. Click Confirm to add these rows.",
                 bg=BG2, fg=FG_DIM, font=("Consolas",8)).pack(side="left", padx=14, pady=6)
        ...
        def _confirm():
            ...
            self.session.exchange_rows = list(self._exchange_rows)
            win.destroy()
            messagebox.showinfo("Imported", f"Added {n_c} compound + {n_e} exchange rows.")
            self._build_output_screen()

        mk_btn(nav, f"✓  Confirm Import", _confirm,
               color=GREEN, fg=BG2, font=("Consolas",10,"bold")).pack(side="right", padx=14, pady=6)
        mk_btn(nav, "Cancel", win.destroy, color=BG4).pack(side="right", padx=4, pady=6)
        win.wait_window()
    ...
    def _build_output_screen(self):
        ...
        hdr = tk.Frame(wrap, bg=BG2); hdr.grid(row=0, column=0, sticky="ew")
        tk.Label(hdr, text="Compound / Exchange Output",
                 font=("Consolas",14,"bold"), bg=BG2, fg=self.ACC, pady=8).pack(side="left", padx=15)
        tk.Label(hdr,
                 text=f"  {len(self._compound_rows)} compound  +  {len(self._exchange_rows)} exchange rows",
        ...
        if self._compound_rows:
            ...
            _add_tab("Compound_Potion rows", cp_xml, "Compound_Potion_rows.xml")
            _add_tab("Compounder_Location rows", cl_xml, "Compounder_Location_rows.xml")
        if self._exchange_rows:
            es_xml = "\n".join(r[0] for r in self._exchange_rows)
            el_xml = "\n".join(r[1] for r in self._exchange_rows)
            _add_tab("ExchangeShopContents rows", es_xml, "ExchangeShopContents_rows.xml")
            _add_tab("Exchange_Location rows", el_xml, "Exchange_Location_rows.xml")

        if not self._compound_rows and not self._exchange_rows:
            tk.Label(wrap, text="No rows yet. Add entries or import a spreadsheet.",
        ...
        mk_btn(nav, "➕  Add Manual Entry", self._build_manual_entry, color=BG4).pack(side="left", padx=4, pady=6)
        mk_btn(nav, "📂  Import Spreadsheet", self._import_from_file, color=BG4).pack(side="left", padx=4, pady=6)
        mk_btn(nav, "🗑  Clear All", self._clear_all, color=BG4).pack(side="left", padx=4, pady=6)
        ...
        def _export():
            ...
            if self._compound_rows:
                exports += [
                    (_APP_SETTINGS["filenames"].get("compound_potion","Compound_Potion.xml"),
                     "\n".join(r[0] for r in self._compound_rows)),
                    (_APP_SETTINGS["filenames"].get("compounder_location","Compounder_Location.xml"),
                     "\n".join(r[1] for r in self._compound_rows)),
                ]
            if self._exchange_rows:
                exports += [
                    (_APP_SETTINGS["filenames"].get("exchange_contents","ExchangeShopContents.xml"),
                     "\n".join(r[0] for r in self._exchange_rows)),
                    (_APP_SETTINGS["filenames"].get("exchange_location","Exchange_Location.xml"),
                     "\n".join(r[1] for r in self._exchange_rows)),
                ]
        ...
        mk_btn(nav, "💾  Export All", _export,
               color=GREEN, fg=BG2, font=("Consolas",10,"bold")).pack(side="right", padx=14, pady=6)

    def _clear_all(self):
        if messagebox.askyesno("Clear", "Clear all compound and exchange rows?"):
            self._compound_rows.clear(); self._exchange_rows.clear()
            self.session.compound_rows = []; self.session.exchange_rows = []
            self._build_start_screen()
...
class CombinedApp(tk.Tk):
    ...
    def _build_layout(self):
        ...
        tk.Label(sidebar, text="MEWSIE'S\nITEMPARAM\nTOOLBOX", font=("Consolas",11,"bold"),
        ...
        sep3 = tk.Frame(sidebar, bg=BG4, height=1); sep3.pack(fill="x", padx=10, pady=(8,4))
        mk_btn(sidebar, "⚙  Settings", lambda: _open_settings_window(self),
               color=BG2, fg=FG_GREY, font=("Consolas",8)).pack(fill="x", padx=8, pady=2)
