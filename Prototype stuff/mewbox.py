...
def _open_settings_window(root):
    ...
    def _build_fn_rows(container_frame, order_list, var_dict, settings_key):
        ...
        items = order_list if fn_sort_var.get() == "tool" else sorted(order_list, key=lambda x: x[2])
...
class XMLBulkUpdater(tk.Frame):
    ...
    def _run(self):
        ...
        # Update RowCount in output text
...
class ToolboxSwapper(tk.Frame):
    ...
    def __init__(self, parent, root, session):
        ...
        self.root = root; self.session = session
        self._src_path   = None
        self._src_type   = None   # "xml_table","xml_raw","csv_table","csv_raw"
        self._tags       = []
        self._table_name = ""
    ...
    def _build(self):
        ...
        tk.Label(self, text="TOOLBOX SWAPPER",
                 font=("Consolas",16,"bold"), bg=BG, fg=self.ACC).pack(pady=(20,2))
        tk.Label(self,
                 text="Convert XML tables → CSV, or CSV → XML table, and back.\n"
                      "Full XML tables (TABLE name= + FIELDINFO headers) are fully supported.\n"
                      "Raw-ROW files and plain CSVs work too — with a heads-up before proceeding.",
                 bg=BG, fg=FG_DIM, font=("Consolas",8), justify="center").pack(pady=(0,10))
        ...
        for lbl, val in [("XML → CSV  (export table to spreadsheet)", "xml_to_csv"),
                         ("CSV → XML  (import CSV back into XML table)", "csv_to_xml")]:
            tk.Radiobutton(s2, text=lbl, variable=self._dir_var, value=val,
                           bg=BG, fg=FG, selectcolor=BG3, activebackground=BG,
                           font=("Consolas",9)).pack(anchor="w", padx=10, pady=2)
        tk.Label(s2,
                 text="  CSV → XML: provide a Base XML in Step 3 to reuse its table header/FIELDINFO. Optional but recommended.",
                 bg=BG, fg=FG_GREY, font=("Consolas",7)).pack(anchor="w", padx=10, pady=(0,6))

        # Step 3: optional base XML
        s3 = mk_section(self, "  Step 3 — Base XML  (CSV → XML only, optional — preserves original table structure)")
    ...
    def _load_csv(self):
        ...
        if not p: return
        head = open(p, encoding="utf-8-sig", errors="replace").read(4096)
        has_hint = bool(re.search(r'TABLE\s+name\s*=', head, re.IGNORECASE) or
                        re.search(r'FIELDINFO\s+Name\s*=', head, re.IGNORECASE))
        headers = []
        try:
        ...
        if not has_hint:
            if not messagebox.askyesno(
                ...
                "This CSV has no TABLE name= or FIELDINFO hint.\n\n"
                "It can still be imported. Load a Base XML in Step 3 to preserve the original table structure.\n\n"
                "Continue anyway?", parent=self.root):
                return
        self._src_path = p; self._src_type = "csv_table" if has_hint else "csv_raw"
        self._tags = headers
        ...
        self._src_type_lbl.set(
            "Type: CSV with table header hint" if has_hint
            else "⚠  Type: Plain CSV (no table header hint)")
        self._dir_var.set("csv_to_xml")

    def _load_base_xml(self):
        p = filedialog.askopenfilename(
            title="Load Base XML (for CSV→XML header)",
            filetypes=[("XML","*.xml"),("All","*.*")], parent=self.root)
        if not p: return
        head = open(p, encoding="utf-8", errors="replace").read(8192)
        if not re.search(r'<TABLE\s+name\s*=', head, re.IGNORECASE):
            messagebox.showwarning("Not a full table",
                "This XML has no TABLE name= header and will not be useful as a base.\n"
    ...
    def _xml_to_csv(self):
        ...
        for row_txt in _iter_xml_rows(self._src_path):
            writer.writerow([(_xml_tag_val(row_txt, t) or "").strip() for t in tags])
            row_count += 1
    ...
    def _csv_to_xml(self):
        ...
        # Build ROW blocks matching toolbox raw output style (no extra indentation)
        row_blocks = []
        for r in csv_rows:
            parts = ["<ROW>"]
            for h in headers:
                ...
                if any(c in val for c in ("<", ">", "&")):
                    parts.append(f"<{h}><![CDATA[{val}]]></{h}>")
                else:
                    parts.append(f"<{h}>{val}</{h}>")
            parts.append("</ROW>")
            row_blocks.append("\n".join(parts))

        if self._base_path:
            base_text = open(self._base_path, encoding="utf-8", errors="replace").read()
            # Strip existing rows, update RowCount, re-insert before </Table>
            stripped = re.sub(r"<ROW>.*?</ROW>", "", base_text, flags=re.DOTALL)
            stripped = re.sub(r"(RowCount\s*=\s*[\"'])(\d+)([\"'])",
                              lambda m: m.group(1)+str(row_count)+m.group(3),
                              stripped, count=1, flags=re.IGNORECASE)
            rows_text = "\n".join(row_blocks) + "\n"
            out_text = re.sub(r"(</Table>)", rows_text + r"\1", stripped, count=1, flags=re.IGNORECASE)
            m = re.search(r"<TABLE\s+name\s*=\s*[\"']([^\"']+)[\"']", base_text, re.IGNORECASE)
            out_fname = (m.group(1) if m else self._table_name) + ".xml"
        else:
            tname = self._table_name
            fi_lines = "\n".join(
                f'<FIELDINFO Name="{h}" IsKey="{"1" if i==0 else "0"}" DataType="STRING"></FIELDINFO>'
                for i, h in enumerate(headers)
            )
            out_fname = tname + ".xml"
            out_text = (
                f'<TABLE name="{tname}" RowCount="{row_count}" TableInfoID="0" FieldCnt="{len(headers)}">\n'
                f"{fi_lines}\n" +
                "\n".join(row_blocks) +
                "\n</Table>"
            )
