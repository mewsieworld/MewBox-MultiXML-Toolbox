...
class Tool1(tk.Frame):
    ...
    def _build_config_screen(self):
        ...
        # Fixed header at top
        hdr = tk.Frame(outer, bg=BG2); hdr.pack(fill="x", side="top")
        ...
        # Fixed nav bar at bottom — must be packed BEFORE the canvas so it stays visible
        nav = tk.Frame(outer, bg=BG2); nav.pack(fill="x", side="bottom")

        # Scrollable content fills remaining space
        scroll_area = tk.Frame(outer, bg=BG); scroll_area.pack(fill="both", expand=True, side="top")
        canvas, container = mk_scroll_canvas(scroll_area)
