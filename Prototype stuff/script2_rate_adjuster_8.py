...
class App(tk.Tk):
    ...
    def _build_manual_screen(self):
        ...
        def save_and_advance():
            # Warn if Random type and DropCnt is empty/zero
            if type_var.get() == 0:
                dc_raw = dc_var.get().strip()
                if not dc_raw or not dc_raw.isdigit() or int(dc_raw) < 1:
                    ans = messagebox.askyesno(
                        "Missed a spot",
                        "Types of Items (DropCnt) is blank or invalid.\n"
                        "It will default to 1. Continue anyway?")
                    if not ans: return
            cfg = gather()
            self.manual_configs[rid] = cfg
    ...
    def _build_output_screen(self, full_xml, csv_rows, count):
        ...
        _exports = [
            ("PresentItemParam2_modified.xml", full_xml),
            ("box_contents_for_script3.csv",   csv_cont),
        ]
