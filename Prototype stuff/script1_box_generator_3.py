...
class BoxGeneratorApp(tk.Tk):
    ...
    def _automate_remaining(self, last_cfg):
        ...
        for i in range(self.current_group_idx, len(self.groups)):
            ...
            proposed = apply_name_template(cfg.get("name_template",""), prev_name, grp["parent_name"])
            proposed = deduplicate_name(proposed, used_names)
            cfg["name"]           = proposed
            cfg["name_template"]  = proposed

            # Comment + Use template substitution
            cfg["comment"] = apply_name_template(cfg.get("comment_template",""), prev_name, grp["parent_name"])
            cfg["use"]     = apply_name_template(cfg.get("use_template",""),     prev_name, grp["parent_name"])
            cfg["comment_template"] = cfg["comment"]
            cfg["use_template"]     = cfg["use"]

            cfg["box_name"] = grp["parent_name"]
            cfg["items"]    = grp["items"]
