...
class Tool18(tk.Frame):
    ...
    def _build_editor(self, prefill=None):
        ...
        def _update_char_display(*_):
            ...
            flag = CHR_FLAG_MAP.get(name, 0)
            fox_note = "  (Fox: no unique libcmgds_category1)" if "Fox" in name else ""
            char_info_lbl.config(
                text=f"  ChrTypeFlags={flag}  goods_cat1={cat1}  "
                     f"goods_char_type={ct}\n"
        ...
        self._filename_var  = tk.StringVar(value=pf.get("filename", ""))
        self._bundlenum_var = tk.StringVar(value=pf.get("bundlenum", "0"))
        self._cmtfilename_var   = tk.StringVar(value=pf.get("cmtfilename", ""))
        self._cmtbundlenum_var  = tk.StringVar(value=pf.get("cmtbundlenum", "0"))
        field(s_fn, "FileName (= InvFileName):", self._filename_var,
              "Applied to FileName AND InvFileName for every piece.\nInvFileName is auto-duplicated from this.")
        field(s_fn, "BundleNum (= InvBundleNum):", self._bundlenum_var,
              "Applied to BundleNum AND InvBundleNum for every piece.\nInvBundleNum is auto-duplicated from this.", w=10)
        field(s_fn, "CmtFileName:", self._cmtfilename_var)
        field(s_fn, "CmtBundleNum:", self._cmtbundlenum_var, w=10)
        tk.Label(s_fn,
                 text="  FileName → duplicated to InvFileName   ·   BundleNum → duplicated to InvBundleNum",
                 bg=BG, fg=FG_GREY, font=("Consolas", 7)).pack(anchor="w", padx=12, pady=(0, 2))

        # Color pattern quick-fill row
        cpf_row = tk.Frame(s_fn, bg=BG); cpf_row.pack(fill="x", padx=10, pady=(2, 6))
        tk.Label(cpf_row, text="Quick-fill pattern:", bg=BG, fg=FG_DIM,
        ...
        _cpv = tk.StringVar(value="— custom —")
        cpf_cb = ttk.Combobox(cpf_row, textvariable=_cpv,
                              values=[p[0] for p in _COLOR_PATTERNS],
                              width=14, state="readonly")
        cpf_cb.pack(side="left", padx=4)

        def _apply_color_pattern(*_):
            chosen = _cpv.get()
            for name, fn_val, bn_val in _COLOR_PATTERNS:
                if name == chosen and fn_val:
                    char = self._char_var.get()
                    # Build path using character name as prefix if FileName is blank
                    prefix = char.lower().replace(" ", "_") + "/"
                    self._filename_var.set(prefix + fn_val)
                    self._bundlenum_var.set(bn_val)
                    break
        cpf_cb.bind("<<ComboboxSelected>>", _apply_color_pattern)

        # Import FileName/BundleNum from CSV for this section
        def _import_fn_csv():
            path = filedialog.askopenfilename(
                title="Import FileName/BundleNum CSV",
            ...
            try:
                with open(path, encoding="utf-8-sig") as f:
                    reader = csv.DictReader(f)
                    for row in reader:
                        rn = {k.strip().lower(): v.strip() for k, v in row.items()}
                        if "filename" in rn:
                            self._filename_var.set(rn["filename"])
                        if "bundlenum" in rn:
                            self._bundlenum_var.set(rn["bundlenum"])
                        if "cmtfilename" in rn:
                            self._cmtfilename_var.set(rn["cmtfilename"])
                        if "cmtbundlenum" in rn:
                            self._cmtbundlenum_var.set(rn["cmtbundlenum"])
                        break  # only use first row
            except Exception as e:
                messagebox.showerror("Import Error", str(e))
        mk_btn(cpf_row, "📂 Import CSV", _import_fn_csv,
               color=BG4, font=("Consolas", 8)).pack(side="left", padx=8)
        ...
        tk.Checkbutton(br, text="Generate Box <ROW>",
                       ...
                       activebackground=BG, font=("Consolas", 9)).pack(side="left")
        field(s_set, "Box Item ID:", self._box_id)
        field(s_set, "Box Name:", self._box_name)
        field(s_set, "Box Comment:", self._box_comment)
        tk.Label(s_set, text="  Set item IDs are auto-filled from the pieces table below.",
        ...
        shop_row_frame = tk.Frame(s_opt, bg=BG)

        def _toggle_shop_fields(*_):
            if self._gen_shop.get():
                shop_row_frame.pack(fill="x", padx=20, pady=2)
            else:
                shop_row_frame.pack_forget()
        ...
        tk.Checkbutton(s_opt, text="Generate R_ShopItem.xml row",
                       ...
                       command=_toggle_shop_fields).pack(anchor="w", padx=12, pady=3)

        # Shop fields — ONLY visible when checkbox is ticked
        ...
        tk.Label(shop_row_frame, text="Count:", bg=BG, fg=FG,
                 font=("Consolas", 9), width=10, anchor="w").pack(side="left")
        tk.Entry(shop_row_frame, textvariable=self._shop_count, width=10, bg=BG3, fg=FG,
                 insertbackground=FG, font=("Consolas", 9), relief="flat").pack(side="left", padx=4)
        tk.Label(shop_row_frame, text="Price override (blank = use pricing above):",
                 bg=BG, fg=FG_GREY, font=("Consolas", 8)).pack(side="left", padx=(6, 0))
        tk.Entry(shop_row_frame, textvariable=self._shop_price_override, width=12, bg=BG3, fg=FG,
                 insertbackground=FG, font=("Consolas", 9), relief="flat").pack(side="left", padx=4)

        # Initially show/hide
        _toggle_shop_fields()

        # Exchange entry (inline, like Tool7 now)
        exch_frame = tk.Frame(s_opt, bg=BG)
        comp_frame = tk.Frame(s_opt, bg=BG)
        ...
        tk.Checkbutton(s_opt, text="Generate Exchange entry",
                       ...
                       activebackground=BG, font=("Consolas", 9),
                       command=_toggle_exchange).pack(anchor="w", padx=12, pady=3)
        exch_lf = tk.LabelFrame(s_opt, text="  Exchange fields  ", bg=BG, fg=ACC2,
                                 font=("Consolas", 8, "bold"), bd=1, relief="groove")
        # exch_frame is inside exch_lf
        ...
        tk.Checkbutton(s_opt, text="Generate Compound entry",
                       ...
                       activebackground=BG, font=("Consolas", 9),
                       command=_toggle_compound).pack(anchor="w", padx=12, pady=3)
        comp_lf = tk.LabelFrame(s_opt, text="  Compound fields  ", bg=BG, fg=ACC1,
                                 font=("Consolas", 8, "bold"), bd=1, relief="groove")
        comp_frame = tk.Frame(comp_lf, bg=BG)
        self._comp_lf = comp_lf

        # Re-wire toggle functions to use LabelFrames
        ...
        # Rebind the checkbuttons with corrected commands
        for w in s_opt.winfo_children():
            if isinstance(w, tk.Checkbutton):
                txt = w.cget("text")
                if "Exchange" in txt:
                    w.config(command=_toggle_exchange2)
                elif "Compound" in txt:
                    w.config(command=_toggle_compound2)
    ...
    def _generate(self):
        ...
        # InvFileName mirrors FileName; InvBundleNum mirrors BundleNum
        inv_fn  = fn
        inv_bn  = bn
