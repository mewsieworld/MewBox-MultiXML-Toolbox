...
def extract_leaf_box_ids(present_text, start_ids):
    """
    For Tool2 (PresentItemParam2 rate adjuster):
    Starting from start_ids, recursively walk boxes until we find
    rows whose drops are ALL non-box items (no PresentItemParam2 row).
    Those are the 'leaf boxes' — the ones whose DropRate/ItemCnt should be adjusted.
    Returns set of leaf-box IDs.
    """
    # Build full present map
    all_present = {}
    for row in ROW_RE.findall(present_text):
        bid = _get_tag(row, "Id")
        if not bid: continue
    ...
    while queue:
        ...
        if not drops:
            # Not in PresentItemParam2 — it's a raw item, not a box row to adjust
            continue
        # Check if all drops are non-boxes (leaf box) or some are boxes (branch)
        has_box_drops = any(d in all_present for d in drops)
        if not has_box_drops:
            # All drops are items — this IS the row to adjust
            leaf_boxes.add(bid)
        else:
            # Some drops are boxes — recurse into the box drops
            for d in drops:
                if d in all_present and d not in visited:
                    visited.add(d)
                    queue.append(d)
            # Also check non-box drops — they might be leaf boxes themselves
            for d in drops:
                if d not in all_present and d not in visited:
                    visited.add(d)
                    # d has no present row — it's a raw item, skip
        ...
        if nested:
            ...
            for row in ROW_RE.findall(self.xml_text):
                ...
                if rid in leaf_box_ids and rid not in seen:
                    nm2, meta2, grp = csv_meta.get(rid, (rid, {}, ""))
