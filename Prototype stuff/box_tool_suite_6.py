...
# ItemParam field headers recognised on import
_IP_FIELD_HEADERS = {
    "name","comment","use","filename","bundlenum","cmtfilename","cmtbundlenum",
    "weight","value","minlevel","money","ncash","tickets","ticket",
    "options","chrtypeflags","chrtypeflag","recycle",
}
# Options keywords that mean "NOT buyable" etc. (positive = restriction active)
_OPT_POSITIVE = {
    "notbuyable","notbuy","nobuy","buy","buyable",
    "notsellable","notsell","nosell","sell","sellable",
    "notexchangeable","notexchange","noexchange","exchange","exchangeable",
    "notpickable","notpick","nopick","pick","pickable",
    "notdroppable","notdrop","nodrop","drop","droppable",
    "notvanishable","notvanish","novanish","vanish","vanishable",
    "noangelinabank","angelinabank","angelina",
    "nolisabank","lisabank","lisa",
}
...
def _is_box_name_header(h):
    ...
    hn = _norm_hdr(h)
    # "Box X" → box name
    if hn.startswith("box") and hn != "boxid": return True
    return bool(h) and hn not in _SKIP_HEADERS and not h.isdigit()

def _parse_options_cell(cell):
    """Parse a cell like 'Not Buyable, Not Sellable' into opt_checks and recycle."""
    opt_checks = [False]*8
    recycle = 0
...
def _parse_chrtypeflags_cell(cell):
    """Parse comma/semicolon separated ChrTypeFlags text, return list of flag ints."""
    flags = []
    for part in re.split(r"[,;|/]+", cell):
        part = part.strip()
        if not part: continue
        if part.isdigit():
            flags.append(int(part)); continue
        f = resolve_chr_flag(part)
        if f is not None: flags.append(f)
    return flags

# Mapping from normalised header to cfg key
_HDR_TO_CFG = {
    ...
    "money":"money","ncash":"ncash_direct","tickets":"ticket","ticket":"ticket",
    "options":"_options_raw","chrtypeflags":"_chr_raw","chrtypeflag":"_chr_raw",
    "recycle":"_recycle_raw","recyclable":"_recycle_raw",
    "boxid":"id","id":"_ambiguous_id",
    "contents":"_contents_raw","content":"_contents_raw",
}

def parse_grouped_csv(text, _ask_id_role=None):
    """Parse a box-definition CSV.
    _ask_id_role: optional callable(col_letter) -> "box"|"contents"
                  used when a lone "ID" header is ambiguous.
    Returns list of group dicts with optional pre-filled cfg fields.
    """
    ...
    # Find all "Box …" columns — these define groups
    box_col_indices = [i for i,h in enumerate(headers) if _is_box_name_header(h)]
    if not box_col_indices: return []

    # Build group spans: each box-name col owns everything from the previous box-name col+1 to itself
    groups_meta = []
    prev_end = -1
    for bc in box_col_indices:
        span = list(range(prev_end+1, bc+1))   # includes the box-name col itself
        groups_meta.append({"box_col": bc, "span": span})
        prev_end = bc

    # For each group, identify what each column in its span does
    results = []
    for gm in groups_meta:
        ...
        # Classify columns in span
        id_col        = None   # box ID column (writes to cfg["id"])
        name_col      = bc     # box name / contents-name column
        contents_col  = None   # explicit "Contents" column
        cfg_cols      = {}     # col_index -> cfg_key

        for ci in span:
            hn = hnorms[ci]
            if ci == bc:
                continue  # box name column
            if hn == "boxid":
                id_col = ci; cfg_cols[ci] = "id"
            elif hn == "_ambiguous_id" or hn == "id":
                # ambiguous lone "ID" — ask if needed
                id_col = ci; cfg_cols[ci] = "_ambiguous_id"
            elif hn in ("contents","content","_contents_raw"):
                contents_col = ci
            elif hn in _HDR_TO_CFG:
                cfg_cols[ci] = _HDR_TO_CFG[hn]
            # else: ignore (rate, level, etc.)

        items = []
        box_cfg_override = {}  # pre-filled fields from CSV columns

        for row in rows[1:]:
            box_val = row[bc].strip() if bc < len(row) else ""
            if not box_val: continue
            # Row carries per-box config values
            for ci, cfg_key in cfg_cols.items():
                if cfg_key in ("id", "_ambiguous_id"): continue
                val = row[ci].strip() if ci < len(row) else ""
                if not val: continue
                if cfg_key == "_options_raw":
                    oc, rv = _parse_options_cell(val)
                    box_cfg_override["opt_checks"] = oc
                    if rv: box_cfg_override["opt_recycle"] = rv
                elif cfg_key == "_chr_raw":
                    flags = _parse_chrtypeflags_cell(val)
                    box_cfg_override["chr_type_flags"] = flags
                elif cfg_key == "_recycle_raw":
                    nv = _norm_hdr(val)
                    if nv in ("recyclable","recycle"): box_cfg_override["opt_recycle"] = 262144
                    elif nv in ("nonrecyclable","norecycle","no"): box_cfg_override["opt_recycle"] = 8388608
                else:
                    box_cfg_override[cfg_key] = val

            # Determine box ID
            box_id = ""
            if id_col is not None:
                raw_id = row[id_col].strip() if id_col < len(row) else ""
                if raw_id.isdigit():
                    box_id = raw_id

            # Parse contents from "Contents" column or items from box_val rows below
            # (The box_val here IS the contents-name if box name col has item names)
            items.append({"id": box_id, "name": box_val,
                          "extra": [], "rate": None, "item_cnt": 1,
                          "cfg_override": dict(box_cfg_override)})

        # Consolidate: if id_col exists, the group has exactly one "parent box" line
        # and the items are the box's contents. Otherwise box_val rows ARE items.
        # Determine if this is a "one row = one box" or "one column = one box" layout.

        # Simple heuristic: if there's an explicit id_col, each row is an item for that box.
        # The group represents ONE box whose name is headers[bc].
        group_box_name = headers[bc]

        # Check for "Contents" column — overrides item name source
        final_items = []
        for rowi, row in enumerate(rows[1:]):
            box_val = row[bc].strip() if bc < len(row) else ""
            if not box_val: continue
            # Get rate from middle cols
            rate = None
            for ci in span:
                if ci == bc or ci in cfg_cols: continue
                v = row[ci].strip() if ci < len(row) else ""
                clean = v.replace("%","").strip()
                try:
                    r = int(clean); rate = r; break
                except: pass
            item_cnt = 1
            for ci in span:
                if hnorms[ci] in ("itemcnt","count"):
                    try: item_cnt = int(row[ci].strip())
                    except: pass
            # Box ID for this item row
            item_id = ""
            if id_col is not None:
                raw_id = row[id_col].strip() if id_col < len(row) else ""
                if raw_id.isdigit(): item_id = raw_id
            # Contents col overrides name
            item_name = box_val
            if contents_col is not None:
                item_name = row[contents_col].strip() if contents_col < len(row) else box_val

            final_items.append({"id": item_id, "name": item_name,
                                 "extra": [], "rate": rate, "item_cnt": item_cnt})

        # Per-group cfg_override: scan non-item rows for cfg data
        group_cfg = {}
        for ci, cfg_key in cfg_cols.items():
            if cfg_key in ("id","_ambiguous_id"): continue
            for row in rows[1:]:
                ...
                if cfg_key == "_options_raw":
                    oc, rv = _parse_options_cell(val)
                    group_cfg["opt_checks"] = oc
                    if rv: group_cfg["opt_recycle"] = rv
                elif cfg_key == "_chr_raw":
                    flags = _parse_chrtypeflags_cell(val)
                    if flags: group_cfg["chr_type_flags"] = flags
                elif cfg_key == "_recycle_raw":
                    nv = _norm_hdr(val)
                    if nv in ("recyclable","recycle"): group_cfg["opt_recycle"] = 262144
                    elif nv in ("nonrecyclable","norecycle","no"): group_cfg["opt_recycle"] = 8388608
                elif cfg_key != "_ambiguous_id":
                    group_cfg[cfg_key] = val
                break  # take first non-empty value

        if final_items:
            results.append({"box_name": group_box_name,
                            "items": final_items,
...
class Tool1(tk.Frame):
    ...
    def _build_config_screen(self):
        ...
        # Apply any field overrides parsed from the CSV
