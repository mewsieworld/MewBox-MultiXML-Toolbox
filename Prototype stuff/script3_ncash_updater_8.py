"""
SCRIPT 3 – NCash / Ticket Cost Updater  (v4)
─────────────────────────────────────────────
Modes:
  Uniform — one Ticket Cost applied to every item in the CSV.
  Manual  — set ticket cost per item; warns if any left blank.

ItemParam: pick any one of the 4 files; siblings auto-loaded.
Each file processed independently. Only files with matches are exported.
...
def build_item_lib(files):
    """files: list of (fname, text). Returns {id_str: name_str}."""
    lib = {}
    for _, text in files:
...
def parse_csv_text(text):
    """
    Accepts:
      1. Script 2 output:  BoxID, BoxName, Item1_ID, Item2_ID, ...
      2. Original box CSV: repeating groups  ID, Level, CharName, ID, Level, CharName, ...
         (DictReader collapses duplicate "ID" headers — must use csv.reader + positional scan)
      3. Simple 2-col:     ID, TicketCost
      4. Single-col:       IDs only
    """
    stripped = text.strip()
    if not stripped:
        return []
    ...
    # ── Format 1: Script 2 output — Item\d+_ID columns ──────────────────
    item_col_positions = [i for i, h in enumerate(raw_headers)
                          if re.match(r'Item\d+_ID', h, re.I)]
    ...
    # ── Format 2: original grouped CSV — repeating "ID" header positions ─
    ...
    # ── Format 3: simple 2-col ID, TicketCost ────────────────────────────
...
class NCashUpdaterApp(tk.Tk):
    def __init__(self):
        ...
        self.csv_items  = []
        self.xml_files  = []   # [(fname, text), ...]  — kept separate per file
        self.item_lib   = {}
    ...
    def _build_manual_screen(self):
        ...
        tk.Label(self, text="Leave blank to skip an item.",
        ...
        def confirm():
            ...
            if blanks:
                ans = messagebox.askyesno(
                    "Missed a spot",
                    f"{len(blanks)} item(s) have no cost and will be SKIPPED:\n\n"
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # PROCESS — per-file, track which file each match came from
    # ─────────────────────────────────────────────────────────────────────────
    def _process_and_show(self):
        ...
        # Run bulk update per file independently
        # file_results: [(fname, modified_text, {id: bool_found}), ...]
        file_results = []
        ...
        # Build per-item result: which file found it (first match wins for dedup)
        # found_in[id] = fname or None
        found_in  = {}
        ...
        for item in self.csv_items:
            ...
            if item["ticket_cost"] is None:
                results.append((iid, name, None, None))   # (id, name, ncash, file_or_None)
            else:
                ncash = updates[iid]
    ...
    def _build_output_screen(self, file_results, results, updates):
        ...
        # ── Per-file XML tabs (only files with at least one match) ────────────
        col_hdr  = f"{'ID':<15}{'Name':<34}{'NCash':<13}Status"
        col_sep  = "─" * 74
        exports  = []   # (fname, content) — only modified files

        for fname, modified_text, found_map in file_results:
            matched_ids = [iid for iid, hit in found_map.items() if hit]
            if not matched_ids:
                continue   # skip entirely — no tab, no export
            exports.append((fname, modified_text))
            short = os.path.splitext(fname)[0]   # label without .xml
            make_tab(short, modified_text, fname)

        # ── Update Log tab ────────────────────────────────────────────────────
        log_parts = []

        # Group results by file, then unmatched/skipped
        for fname, _, found_map in file_results:
            matched_ids = {iid for iid, hit in found_map.items() if hit}
            file_rows   = [(iid, name, ncash, ffile)
                           for iid, name, ncash, ffile in results
                           if ffile == fname]
            if not file_rows:
                log_parts.append(
                    f"{fname}  →  No matching IDs — Skipped file!\n")
                continue
            log_parts.append(
                f"{fname}  →  {len(file_rows)} ID(s) matching CSV")
            log_parts.append(f"  " + ", ".join(r[0] for r in file_rows))
            log_parts.append(f"  {col_hdr}")
            log_parts.append(f"  {col_sep}")
            for iid, name, ncash, _ in file_rows:
                name_str = (name or "—")[:32]
                log_parts.append(f"  {iid:<15}{name_str:<34}{ncash:<13}✓ Updated")
            log_parts.append("")

        # Unassigned / skipped section
        unassigned = [(iid, name, ncash, ffile)
                      for iid, name, ncash, ffile in results
                      if ffile is None]
        ...
        if missing_rows:
            ...
            log_parts.append(f"  " + ", ".join(r[0] for r in missing_rows))
            log_parts.append(f"  {col_hdr}")
            log_parts.append(f"  {col_sep}")
            for iid, name, ncash in missing_rows:
                name_str = (name or "—")[:32]
                log_parts.append(f"  {iid:<15}{name_str:<34}{ncash:<13}⚠ Not found")
            log_parts.append("")
        if skipped_rows:
            log_parts.append(f"  — Skipped (no cost entered): {len(skipped_rows)} ID(s)")
            log_parts.append(f"  " + ", ".join(r[0] for r in skipped_rows))
            log_parts.append(f"  {col_hdr}")
            log_parts.append(f"  {col_sep}")
            for iid, name in skipped_rows:
                name_str = (name or "—")[:32]
                log_parts.append(f"  {iid:<15}{name_str:<34}{'—':<13}SKIPPED")
        ...
        def export_all():
            ...
            for efname, content in exports:
                path = os.path.join(folder, efname)
