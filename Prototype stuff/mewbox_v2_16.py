...
class Tool18(tk.Frame):
    ...
    def _do_launch_itemparam(self):
        """Switch to ItemParam Generator pre-filled for the first fashion piece."""
        pieces = []
        for rf, vid, vn, vcmt in self._item_row_widgets:
            if rf.winfo_exists() and vn.get().strip():
                pfn = getattr(vid, "_part_fn", "") or self._partfile_shared_var.get().strip()
        ...
        if not pieces:
            messagebox.showwarning("No Pieces",
                "Add fashion pieces in ⑥ Fashion Pieces before generating.", parent=self.root)
            return
        char = self._char_var.get()
        chr_flag = CHR_FLAG_MAP.get(char, 0)
        color = getattr(self, "_color_name_var", tk.StringVar()).get().strip()
        ...
        bn  = self._bundlenum_var.get().strip() or "0"
        cbn = self._cmtbundlenum_var.get().strip() or "0"
        opt_flags = [str(fv) for (_, fv), v in zip(OPTIONS_CHECKS,
                     getattr(self, "_fash_opt_checks", [])) if v.get()]
        opt_str = "/".join(["2", "16"] + opt_flags)
        recycle = getattr(self, "_fash_recycle_var", tk.IntVar()).get()
        ncash_val = "0"
        if recycle == 262144:
            try:
                tv = float(getattr(self, "_fash_ticket_var", tk.StringVar(value="0")).get() or "0")
                mode = getattr(self, "_fash_ncash_input_mode", tk.StringVar(value="tickets")).get()
                ncash_val = str(round(tv * 133) if mode == "tickets" else int(tv))
            except: pass
        ...
        cfg = {
            ...
            "_fashion_char":       char,
            "_fashion_chr_flag":   chr_flag,
            "_fashion_fn":         fn, "_fashion_bn": bn,
            "_fashion_cmt":        cmt, "_fashion_cbn": cbn,
            "_fashion_recycle":    recycle,
            "_fashion_ncash":      ncash_val,
            "_fashion_galder":     galder,
            "_fashion_shopfile":   _sanitise_xml_path(self._shopfile_var.get().strip()),
    ...
    def _do_launch_set(self):
        ...
        set_name = self._set_name.get().strip()
        if not set_id:
            messagebox.showwarning("No Set ID", "Enter a Set ID in ④ Set & Box first.",
    ...
    def _do_launch_box(self):
        box_id = self._box_id.get().strip()
        if not box_id:
            messagebox.showwarning("No Box ID", "Enter a Box ID in ④ Set & Box first.",
                                   parent=self.root); return
        items = []
        for rf, vid, vn, vcmt in self._item_row_widgets:
            if rf.winfo_exists() and vid.get().strip():
